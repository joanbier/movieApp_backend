...
internal class RemoveFavoriteCommandHandler : ICommandHandler<RemoveFavoriteCommand>
{
    ...
    public RemoveFavoriteCommandHandler(IUserFavoriteRepository userFavoriteRepository, IMovieRepository movieRepository, 
    ...
    public async Task Handle(RemoveFavoriteCommand request, CancellationToken cancellationToken)
    {
        ...
        if (isAlreadyFavorite)
        {
            ...
            _userFavoriteRepository.RemoveFavorite(favorite);
            await _unitOfWork.SaveChangesAsync(cancellationToken);
        }
        else
        {
            throw new MovieAsFavNotFoundException(request.MovieId);
        }
